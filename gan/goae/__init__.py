...
if __name__ == "__main__":
    ...
    def image_to_tensor(i, size=256):  #PIL图像转换为PyTorch张量，用于神经网络处理。
        ...
        i = np.array(i)
        i = i.transpose(2, 0, 1)
        i = torch.from_numpy(i).to(torch.float32).to("cuda:6") / 127.5 - 1
        return i
    ...
    goae.load_state_dict(torch.load(args.encoder_ckpt, map_location="cuda:6"))  #加载预训练权重到编码器模型
    goae = goae.to("cuda:6")
    # goae.eval()
    ...
    w = w + torch.load("./files/w_avg_ffhqrebalanced512-128.pt").unsqueeze(0).to("cuda:6")  #使用torch.load加载预训练的平均潜在向量 使用unsqueeze(0)添加批次维度，使其可以与编码器输出的潜在向量w结合

    with dnnlib.util.open_url("./files/ffhqrebalanced512-128.pkl") as f:
        g_source = legacy.load_network_pkl(f)["G_ema"].to("cuda:6")  #加载预训练的生成器模型并获取其中的G_ema（指数移动平均生成器）
    # 定义相机的内参（intrinsics），这通常包括焦距、主点坐标等
    # intrinsics = FOV_to_intrinsics(18.837, device="cuda") # from eg3d
    intrinsics = torch.tensor([4.2647, 0, 0.5, 0, 4.2647, 0.5, 0, 0, 1], device="cuda:6") # from goae


    # cam_pivot = torch.tensor(g_source.rendering_kwargs.get("avg_cam_pivot", [0, 0, 0]), device="cuda") # from eg3d
    cam_pivot = torch.tensor([0, 0, 0.2], device="cuda:6") # from goae
